...
class Robot(var storage:Array[Int],var target:String,var expertise:Array[Int]){
    var Carry = new ListBuffer[Sample]()

    def this(storage:Array[Int],target:String,expertise:Array[Int],Carry:ListBuffer[Sample]){
        this(storage,target,expertise)
        this.Carry = Carry
...
object Player extends App {
    ...
    while(true) {
        ...
        for(i <- 0 until 2) {
            ...
            robots+= new Robot(Array(storageA,storageB,storageC,storageD,storageE),target,Array(expertiseA,expertiseB,expertiseC,expertiseD,expertiseE)) 
        ...
        //taşımıyorsam
        if(me.Carry.isEmpty){
            if(me.expertise.sum < 7){
                connectNgo("SAMPLES",2,me.target)}
                else{
                    connectNgo("SAMPLES",3,me.target)
                }            
        }else{
            ...
            if(sample.diag){
                ...
                breakable{
                    for(i<- 0 until 5){
                        if(me.storage(i) < (sample.costs(i) - (me.expertise(i)) )  ){
                            need = "ABCDE"(i)
                            //println(need)
                            break()
                            }
